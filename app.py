...
def signature_verify(signature_image: UploadFile = File(...), database_image: UploadFile = File(...)):
    ...
    try:
        ...
        if score is None or math.isnan(score) or score < 0 or score > 1:
            return JSONResponse({
                "score": None,
                "result": "error",
        ...
        # Log actual error in real deployment, here just return it
        return JSONResponse({
            "score": None,
            "result": "error",
            "detail": str(e)
...
def predict_similarity(image1_path, image2_path):
    ...
    print(f"Raw prediction score: {raw_score}")  # Debug print
    try:
